...
dim AddnewItemToPP
orgcart=arr
AddSpecialPricing=false
AddnewItemToPP=1
Flavoured =False
For i=0 To UBound(arr)
	
	if((arr(i,0)=3) and (AddnewItemToPP=1) ) then
		if(isnewcustomer()) then
			IF(AddSpecialPricing=false) THEN arr(i,3)=arr(i,3)-1	
			AddSpecialPricing=true		
			if(arr(i,3)=0) then 
				AddnewItemToPP=0
				arr(i,0)= 4
 				arr(i,1)= "VSL#3 Unflavoured 30 sachets(new customer)"
 				arr(i,2)= 99
 				arr(i,3)= 1
				'AddnewItemToPP=0
			end if	
	...
	if((arr(i,0)=2) and (AddnewItemToPP=1)) then
		if(isnewcustomer()) then
			IF(AddSpecialPricing=false) THEN arr(i,3)=arr(i,3)-1	
			AddSpecialPricing=true		
			Flavoured =true
			if(arr(i,3)=0) then 
				...
				AddnewItemToPP=0
				arr(i,0)= 1
 				arr(i,1)= "VSL#3 Flavoured 30 sachets(new customer)"
 				arr(i,2)= 99
 				arr(i,3)= 1
				'AddnewItemToPP=0
			end if	
...
'arr(i,3)'qty
next
if (AddSpecialPricing ) then
if ( AddnewItemToPP=1 ) then
	ReDim arrTemp(Ubound(arr)+1,5)
	For i=0 To UBound(arr)
	...
	if(Flavoured) THEN
		...
		arrTemp(UBound(arrTemp),0)= 1
 		arrTemp(UBound(arrTemp),1)= "VSL#3 Flavoured 30 sachets(new customer)"
 		arrTemp(UBound(arrTemp),2)= 99
 		arrTemp(UBound(arrTemp),3)= 1
	
	else
	
		arrTemp(UBound(arrTemp),0)= 4
 		arrTemp(UBound(arrTemp),1)= "VSL#3 Unflavoured 30 sachets(new customer)"
 		arrTemp(UBound(arrTemp),2)= 99
 		arrTemp(UBound(arrTemp),3)= 1
	END IF
...
discountAmount=0
discountAmount = Session("discount_am")
...
'******** Anniversary sale***********************
		if(Session("Anniversary_sale")) then
			...
			if(int(totalCnt/3) >0) then
				k=k+1
%>
			<input type="hidden" name="item_name_<%=UBound(arr)+k%>" value="Anniversary Sale Free box">
			<input type="hidden" name="amount_<%=UBound(arr)+k%>" value="0">
			<input type="hidden" name="quantity_<%=UBound(arr)+k%>" size="3" value="<%=int(totalCnt/3 )%>"><br>
			<!--<input type="hidden" name="shipping_<%=UBound(arr)+k%>" size="3" value="0"><br>-->
<%			InstQ = "INSERT INTO OrderDetails(OrderId,ProductId,Quantity,Price) " 
			InstQ = InstQ & "VALUES(" & OrderNum & ",5, " & int(totalCnt/3 ) & ",0)" 
				c.execute InstQ	
			end if
