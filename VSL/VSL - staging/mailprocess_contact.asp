...
honey=Request("honeyPot")
sPage=Request("RequestPage")
sFrom = Request("firstName")
response.write sfrom

If blnCAPTCHAcodeCorrect = False Then 
	response.Redirect("contact-us.asp?e=ca&" & Request.Form)
	Call stopProcess()
	'else
	'response.write "valid code"
end if

if(honey <> "") then
	Response.Redirect ("contact-us.asp?err=bt")
end if	
...
strTo ="medicalinformation@ferring.com" 'Make sure the From field has no spaces. 
...
if Mail.SendMail then
	...
	if(sPage="F") then
		Response.Redirect ("french/thankyou.html")
	else		
