...
%></td>
	...
	sSql = "SELECT DiscountTypes.DiscountType, DiscountTypes.DiscountTypeId, Discountcodes.used, Count(Discountcodes.Discountid) AS CountOfDiscountid, Orders.payment_status "
	sSql=sSql & " FROM (Orders RIGHT JOIN Discountcodes ON Orders.PromoCodeUsed = Discountcodes.DiscountCode) INNER JOIN DiscountTypes ON Discountcodes.DiscountTypeId = DiscountTypes.DiscountTypeId "
	sSql=sSql & " WHERE (((Orders.payment_status)=""Completed"")) GROUP BY DiscountTypes.DiscountType, DiscountTypes.DiscountTypeId, Discountcodes.used, Orders.payment_status "
	sSql=sSql & " ORDER BY DiscountTypes.DiscountTypeId, Discountcodes.used;"
	...
	i=0
	Do While not rs.EOF
			p(i,0)= rs("DiscountTypeId") 
			p(i,1)= rs("DiscountType") 
			p(i,2)= rs("CountOfDiscountid") 
			p(i,3)= 0
			rs.MoveNext		
			i=i+1			
	Loop
	...
	sSql = "SELECT DiscountTypes.DiscountType, DiscountTypes.DiscountTypeId, Count(Discountcodes.Discountid) AS CountOfDiscountid "
	sSql=sSql & " FROM Discountcodes INNER JOIN DiscountTypes ON Discountcodes.DiscountTypeId = DiscountTypes.DiscountTypeId "
	sSql=sSql & " WHERE (used=false)  GROUP BY DiscountTypes.DiscountType, DiscountTypes.DiscountTypeId "
	sSql=sSql & " ORDER BY DiscountTypes.DiscountTypeId;"
	rs.open sSql,conn	
	i=0
	Do While not rs.EOF
			p(i,3)= p(i,3) +rs("CountOfDiscountid") 
			rs.MoveNext		
			i=i+1					
	Loop
	...
	For i = 0 to UBound(p, 2) -1
		strHtml = strHtml &  "<tr>"
		
		strHtml = strHtml & "<td align='middle' valign='top'>" & p(i,1) & "</td>"

		'cn=rs("CountOfDiscountid")
		strHtml = strHtml & "<td align='middle' valign='top'>" & p(i,2) & "</td>"
		
		'rs.MoveNext	
		'totcnt=rs("CountOfDiscountid")
		
		strHtml = strHtml & "<td align='middle' valign='top'>" & p(i,3) & "</td>"
			strHtml = strHtml & "<td align='middle' valign='top'>" & Round((p(i,2)/(p(i,2)+p(i,3)))*100,1) & "%</td>"
		'totcnt=1000
		'if(rs("DiscountTypeId") =2) then totcnt=2100
		'strHtml = strHtml & "<td align='middle' valign='top'><a href='Discountcodeslist.asp?t=Discountcodes&z_DiscountTypeId=%3D&x_DiscountTypeId=" &  rs("DiscountTypeId") & "&psearch=&Submit=Search+(*)&psearchtype=&order=OrderId&ordertype=DESC'> " & (totcnt-rs("CountOfDiscountid")) & "</a></td>"
		'strHtml = strHtml & "<td align='middle' valign='top'>" & round(((totcnt-rs("CountOfDiscountid"))/totcnt)*100 ,2) & "%</td>"
		strHtml = strHtml &  "</tr>"
