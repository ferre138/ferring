...
Call Orders_add.Page_Init()

' Page main processing
Call Orders_add.Page_Main()
%>
...
<p class="aspmaker ewTitle"><%= Language.Phrase("Add") %>&nbsp;<%= Language.Phrase("TblTypeTABLE") %><%= Orders.TableCaption %></p>
<p class="aspmaker"><a href="<%= Orders.ReturnUrl %>"><%= Language.Phrase("GoBack") %></a></p>
<% Orders_add.ShowMessage %>
<form name="fOrdersadd" id="fOrdersadd" action="<%= ew_CurrentPage %>" method="post" onsubmit="return Orders_add.ValidateForm(this);">
<p>
...
<input type="text" name="x_EmailSent" id="x_EmailSent" size="30" maxlength="255" value="<%= Orders.EmailSent.EditValue %>"<%= Orders.EmailSent.EditAttributes %>>
...
<p>
<% If Orders.CurrentDetailTable = "OrderDetails" And OrderDetails.DetailAdd Then %>
<br>
...
Class cOrders_add
	...
	Sub Page_Main()
		...
		Else
			...
			If Request.QueryString("OrderId").Count > 0 Then
				...
				Call Orders.SetKey("OrderId", Orders.OrderId.CurrentValue) ' Set up key
			Else
				Call Orders.SetKey("OrderId", "") ' Clear key
				CopyRecord = False
			End If
			If CopyRecord Then
		...
		Select Case Orders.CurrentAction
			...
			Case "A" ' Add new record
				...
				If AddRow(OldRecordset) Then ' Add successful
					...
					Dim sReturnUrl
					If Orders.CurrentDetailTable <> "" Then ' Master/detail add
	...
	Function LoadDefaultValues()
		...
		Orders.InvoiceId.CurrentValue = Null
		Orders.InvoiceId.OldValue = Orders.InvoiceId.CurrentValue
		...
		Orders.Ship_Address.CurrentValue = Null
		Orders.Ship_Address.OldValue = Orders.Ship_Address.CurrentValue
		Orders.Ship_Address2.CurrentValue = Null
		Orders.Ship_Address2.OldValue = Orders.Ship_Address2.CurrentValue
		Orders.Ship_City.CurrentValue = Null
		Orders.Ship_City.OldValue = Orders.Ship_City.CurrentValue
		Orders.Ship_Province.CurrentValue = Null
		Orders.Ship_Province.OldValue = Orders.Ship_Province.CurrentValue
		Orders.Ship_Postal.CurrentValue = Null
		Orders.Ship_Postal.OldValue = Orders.Ship_Postal.CurrentValue
		Orders.Ship_Country.CurrentValue = Null
		Orders.Ship_Country.OldValue = Orders.Ship_Country.CurrentValue
		Orders.Ship_Phone.CurrentValue = Null
		Orders.Ship_Phone.OldValue = Orders.Ship_Phone.CurrentValue
		Orders.Ship_Email.CurrentValue = Null
		Orders.Ship_Email.OldValue = Orders.Ship_Email.CurrentValue
		Orders.payment_status.CurrentValue = "WIP"
		Orders.Ordered_Date.CurrentValue = Now()
		Orders.payment_date.CurrentValue = Null
		Orders.payment_date.OldValue = Orders.payment_date.CurrentValue
		Orders.pfirst_name.CurrentValue = Null
		Orders.pfirst_name.OldValue = Orders.pfirst_name.CurrentValue
		Orders.plast_name.CurrentValue = Null
		Orders.plast_name.OldValue = Orders.plast_name.CurrentValue
		Orders.payer_email.CurrentValue = Null
		Orders.payer_email.OldValue = Orders.payer_email.CurrentValue
		Orders.txn_id.CurrentValue = Null
		Orders.txn_id.OldValue = Orders.txn_id.CurrentValue
		...
		Orders.payment_type.CurrentValue = Null
		Orders.payment_type.OldValue = Orders.payment_type.CurrentValue
		Orders.txn_type.CurrentValue = Null
		Orders.txn_type.OldValue = Orders.txn_type.CurrentValue
		Orders.receiver_email.CurrentValue = Null
		Orders.receiver_email.OldValue = Orders.receiver_email.CurrentValue
		Orders.pShip_Name.CurrentValue = Null
		Orders.pShip_Name.OldValue = Orders.pShip_Name.CurrentValue
		Orders.pShip_Address.CurrentValue = Null
		Orders.pShip_Address.OldValue = Orders.pShip_Address.CurrentValue
		Orders.pShip_City.CurrentValue = Null
		Orders.pShip_City.OldValue = Orders.pShip_City.CurrentValue
		Orders.pShip_Province.CurrentValue = Null
		Orders.pShip_Province.OldValue = Orders.pShip_Province.CurrentValue
		Orders.pShip_Postal.CurrentValue = Null
		Orders.pShip_Postal.OldValue = Orders.pShip_Postal.CurrentValue
		Orders.pShip_Country.CurrentValue = Null
		Orders.pShip_Country.OldValue = Orders.pShip_Country.CurrentValue
	...
	Function AddRow(RsOld)
		...
		' Field InvoiceId
		Call Orders.InvoiceId.SetDbValue(Rs, Orders.InvoiceId.CurrentValue, Null, False)
		...
		' Field Ship_Address
		Call Orders.Ship_Address.SetDbValue(Rs, Orders.Ship_Address.CurrentValue, Null, False)

		' Field Ship_Address2
		Call Orders.Ship_Address2.SetDbValue(Rs, Orders.Ship_Address2.CurrentValue, Null, False)

		' Field Ship_City
		Call Orders.Ship_City.SetDbValue(Rs, Orders.Ship_City.CurrentValue, Null, False)

		' Field Ship_Province
		Call Orders.Ship_Province.SetDbValue(Rs, Orders.Ship_Province.CurrentValue, Null, False)

		' Field Ship_Postal
		Call Orders.Ship_Postal.SetDbValue(Rs, Orders.Ship_Postal.CurrentValue, Null, False)

		' Field Ship_Country
		Call Orders.Ship_Country.SetDbValue(Rs, Orders.Ship_Country.CurrentValue, Null, False)

		' Field Ship_Phone
		Call Orders.Ship_Phone.SetDbValue(Rs, Orders.Ship_Phone.CurrentValue, Null, False)

		' Field Ship_Email
		Call Orders.Ship_Email.SetDbValue(Rs, Orders.Ship_Email.CurrentValue, Null, False)
		...
		' Field payment_date
		Call Orders.payment_date.SetDbValue(Rs, Orders.payment_date.CurrentValue, Null, False)

		' Field pfirst_name
		Call Orders.pfirst_name.SetDbValue(Rs, Orders.pfirst_name.CurrentValue, Null, False)

		' Field plast_name
		Call Orders.plast_name.SetDbValue(Rs, Orders.plast_name.CurrentValue, Null, False)

		' Field payer_email
		Call Orders.payer_email.SetDbValue(Rs, Orders.payer_email.CurrentValue, Null, False)

		' Field txn_id
		Call Orders.txn_id.SetDbValue(Rs, Orders.txn_id.CurrentValue, Null, False)
		...
		' Field payment_type
		Call Orders.payment_type.SetDbValue(Rs, Orders.payment_type.CurrentValue, Null, False)

		' Field txn_type
		Call Orders.txn_type.SetDbValue(Rs, Orders.txn_type.CurrentValue, Null, False)

		' Field receiver_email
		Call Orders.receiver_email.SetDbValue(Rs, Orders.receiver_email.CurrentValue, Null, False)

		' Field pShip_Name
		Call Orders.pShip_Name.SetDbValue(Rs, Orders.pShip_Name.CurrentValue, Null, False)

		' Field pShip_Address
		Call Orders.pShip_Address.SetDbValue(Rs, Orders.pShip_Address.CurrentValue, Null, False)

		' Field pShip_City
		Call Orders.pShip_City.SetDbValue(Rs, Orders.pShip_City.CurrentValue, Null, False)

		' Field pShip_Province
		Call Orders.pShip_Province.SetDbValue(Rs, Orders.pShip_Province.CurrentValue, Null, False)

		' Field pShip_Postal
		Call Orders.pShip_Postal.SetDbValue(Rs, Orders.pShip_Postal.CurrentValue, Null, False)

		' Field pShip_Country
		Call Orders.pShip_Country.SetDbValue(Rs, Orders.pShip_Country.CurrentValue, Null, False)
		...
		If AddRow Then
			If Orders.CurrentDetailTable = "OrderDetails" And OrderDetails.DetailAdd Then
				OrderDetails.OrderId.SessionValue = Orders.OrderId.CurrentValue ' Set master key
				Dim OrderDetails_grid
				Set OrderDetails_grid = New cOrderDetails_grid ' get detail page object
				AddRow = OrderDetails_grid.GridInsert
				Set OrderDetails_grid = Nothing
			End If
		End If
	...
	Sub SetUpDetailParms()
		...
		If sDetailTblVar <> "" Then
			If sDetailTblVar = "OrderDetails" Then
				...
				End If
				If OrderDetails.DetailAdd Then
					If CopyRecord Then
						OrderDetails.CurrentMode = "copy"
					Else
						OrderDetails.CurrentMode = "add"
					End If
					OrderDetails.CurrentAction = "gridadd"
