...
' sToEmail = request.QueryString("emailTo")
...
strSQL = "SELECT payer_email,pShip_Name,pShip_Address,pShip_City,pShip_Province,pShip_Postal,pShip_Country, productId,Quantity, Amount, Tax, Shipping FROM orders,orderdetails "
...
Do While not rst.EOF
	...
	if counter = 0 Then
		...
		pShipName = rst("pShip_Name")
		pShipAddr = rst("pShip_Address")
		pShipCity = rst("pShip_City")
		pShipProv = rst("pShip_Province")
		pShipPostal = rst("pShip_Postal")
		pShipCountry = rst("pShip_Country")
	End if 
	strHtml = strHtml & "<tr height=25px>"
	...
	strHtml = strHtml & "<td align='center' valign='top'>$" & rs("Price")  & "</td>"
	strHtml = strHtml & "<td align='right' valign='top'>$" & FormatNumber(rs("Price") * rst("Quantity"),2) & "</td>"
	strHtml = strHtml & "</tr>"
	counter = 1
	rs.close
	rst.MoveNext						
...
<form id="formEmail" name="formEmail" method="post" action="sendEmail.asp">
  <table width="100%" border="0">
    ...
    <tr>
      <td>&nbsp;</td>
      <td><textarea name="EmailText" cols="100" rows="30" id="EmailText"><%=sMail%></textarea></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
