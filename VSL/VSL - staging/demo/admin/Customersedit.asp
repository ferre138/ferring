...
<!--#include file="Ordersgridcls.asp" -->
...
' Define page object
Dim Customers_edit
Set Customers_edit = New cCustomers_edit
Set Page = Customers_edit

' Page init processing
Call Customers_edit.Page_Init()

' Page main processing
Call Customers_edit.Page_Main()
%>
...
Customers_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
...
<% Customers_edit.ShowPageHeader() %>
<p class="aspmaker ewTitle"><%= Language.Phrase("Edit") %>&nbsp;<%= Language.Phrase("TblTypeTABLE") %><%= Customers.TableCaption %></p>
<p class="aspmaker"><a href="<%= Customers.ReturnUrl %>"><%= Language.Phrase("GoBack") %></a></p>
<% Customers_edit.ShowMessage %>
<form name="fCustomersedit" id="fCustomersedit" action="<%= ew_CurrentPage %>" method="post" onsubmit="return Customers_edit.ValidateForm(this);">
<p>
...
<% If Customers.CustomerID.Visible Then ' CustomerID %>
	<tr id="r_CustomerID"<%= Customers.RowAttributes %>>
		...
		<td<%= Customers.CustomerID.CellAttributes %>><span id="el_CustomerID">
<div<%= Customers.CustomerID.ViewAttributes %>><%= Customers.CustomerID.EditValue %></div>
<input type="hidden" name="x_CustomerID" id="x_CustomerID" value="<%= Server.HTMLEncode(Customers.CustomerID.CurrentValue&"") %>">
</span><%= Customers.CustomerID.CustomMsg %></td>
	</tr>
<% End If %>
...
<p>
<% If Customers.CurrentDetailTable = "Orders" And Orders.DetailEdit Then %>
<br>
<!--#include file="Ordersgrid.asp" -->
<br>
<% End If %>
...
Class cCustomers_edit
	...
	Sub Page_Main()
		...
		' Check if valid key
		If Customers.CustomerID.CurrentValue = "" Then Call Page_Terminate("Customerslist.asp") ' Invalid key, return to list
		...
		Select Case Customers.CurrentAction
			...
			Case "U" ' Update
				...
				If EditRow() Then ' Update Record based on key
					...
					Dim sReturnUrl
					If Customers.CurrentDetailTable <> "" Then ' Master/detail edit
						sReturnUrl = Customers.DetailUrl
					Else
	...
	Function LoadFormValues()
		...
		If Not Customers.CustomerID.FldIsDetailKey Then Customers.CustomerID.FormValue = ObjForm.GetValue("x_CustomerID")
	...
	Function ValidateForm()
		...
		If Customers.CurrentDetailTable = "Orders" And Orders.DetailEdit Then
			Dim Orders_grid
			Set Orders_grid = new cOrders_grid ' get detail page object
			Call Orders_grid.ValidateGridForm()
	...
	Function EditRow()
		...
		Else
			...
			' Begin transaction
			If Customers.CurrentDetailTable <> "" Then Conn.BeginTrans

			' Field Inv_FirstName
			Call Customers.Inv_FirstName.SetDbValue(Rs, Customers.Inv_FirstName.CurrentValue, Null, Customers.Inv_FirstName.ReadOnly)

			' Field Inv_LastName
			Call Customers.Inv_LastName.SetDbValue(Rs, Customers.Inv_LastName.CurrentValue, Null, Customers.Inv_LastName.ReadOnly)

			' Field Inv_Address
			Call Customers.Inv_Address.SetDbValue(Rs, Customers.Inv_Address.CurrentValue, Null, Customers.Inv_Address.ReadOnly)

			' Field inv_City
			Call Customers.inv_City.SetDbValue(Rs, Customers.inv_City.CurrentValue, Null, Customers.inv_City.ReadOnly)

			' Field inv_Province
			Call Customers.inv_Province.SetDbValue(Rs, Customers.inv_Province.CurrentValue, Null, Customers.inv_Province.ReadOnly)

			' Field inv_PostalCode
			Call Customers.inv_PostalCode.SetDbValue(Rs, Customers.inv_PostalCode.CurrentValue, Null, Customers.inv_PostalCode.ReadOnly)

			' Field inv_Country
			Call Customers.inv_Country.SetDbValue(Rs, Customers.inv_Country.CurrentValue, Null, Customers.inv_Country.ReadOnly)

			' Field inv_PhoneNumber
			Call Customers.inv_PhoneNumber.SetDbValue(Rs, Customers.inv_PhoneNumber.CurrentValue, Null, Customers.inv_PhoneNumber.ReadOnly)

			' Field inv_EmailAddress
			Call Customers.inv_EmailAddress.SetDbValue(Rs, Customers.inv_EmailAddress.CurrentValue, Null, Customers.inv_EmailAddress.ReadOnly)

			' Field Notes
			Call Customers.Notes.SetDbValue(Rs, Customers.Notes.CurrentValue, Null, Customers.Notes.ReadOnly)

			' Field inv_Fax
			Call Customers.inv_Fax.SetDbValue(Rs, Customers.inv_Fax.CurrentValue, Null, Customers.inv_Fax.ReadOnly)

			' Field Inv_Address2
			Call Customers.Inv_Address2.SetDbValue(Rs, Customers.Inv_Address2.CurrentValue, Null, Customers.Inv_Address2.ReadOnly)

			' Field UserName
			Call Customers.UserName.SetDbValue(Rs, Customers.UserName.CurrentValue, Null, Customers.UserName.ReadOnly)

			' Field passwrd
			Call Customers.passwrd.SetDbValue(Rs, Customers.passwrd.CurrentValue, Null, Customers.passwrd.ReadOnly)

			' Field NewCustomer
			boolwrk = Customers.NewCustomer.CurrentValue
			If boolwrk&"" <> "1" And boolwrk&"" <> "0" Then boolwrk = ew_IIf(boolwrk&"" <> "", "1", "0")
			Call Customers.NewCustomer.SetDbValue(Rs, boolwrk, Null, Customers.NewCustomer.ReadOnly)
			...
			If bUpdateRow Then
				...
				If EditRow Then
					If Customers.CurrentDetailTable = "Orders" And Orders.DetailEdit Then
						Dim Orders_grid
						Set Orders_grid = New cOrders_grid ' get detail page object
						EditRow = Orders_grid.GridUpdate
						Set Orders_grid = Nothing
					End If
				End If

				' Commit/Rollback transaction
				If Customers.CurrentDetailTable <> "" Then
					If EditRow Then
	...
	Sub SetUpDetailParms()
		...
		If Request.QueryString(EW_TABLE_SHOW_DETAIL).Count > 0 Then
			...
			Customers.CurrentDetailTable = sDetailTblVar
		Else
			sDetailTblVar = Customers.CurrentDetailTable
		End If
		If sDetailTblVar <> "" Then
			If sDetailTblVar = "Orders" Then
				If IsEmpty(Orders) Then
					Set Orders = New cOrders
				End If
				If Orders.DetailEdit Then
					Orders.CurrentMode = "edit"
					Orders.CurrentAction = "gridedit"

					' Save current master table to detail table
					Orders.CurrentMasterTable = Customers.TableVar
					Orders.StartRecordNumber = 1
					Orders.CustomerId.FldIsDetailKey = True
					Orders.CustomerId.CurrentValue = Customers.CustomerID.CurrentValue
					Orders.CustomerId.SessionValue = Orders.CustomerId.CurrentValue
				End If
			End If
		End If
	End Sub
