...
Class cOrders
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "PromoCodeUsed", PromoCodeUsed)
	End Sub
	...
	Public Property Get MasterTable()
		If CurrentMasterTable = "Customers" Then
			Set MasterTable = Customers
			Exit Property
		End If
	...
	Public Property Get MasterFilter()
		...
		sMasterFilter = ""
		If CurrentMasterTable = "Customers" Then
			If CustomerId.SessionValue <> "" Then
				sMasterFilter = sMasterFilter & "[CustomerID]=" & ew_QuotedValue(CustomerId.SessionValue, EW_DATATYPE_NUMBER)
			Else
				MasterFilter = ""
				Exit Property
			End If
	...
	Public Property Get DetailFilter()
		...
		If CurrentMasterTable = "Customers" Then
			If CustomerId.SessionValue <> "" Then
				sDetailFilter = sDetailFilter & "[CustomerId]=" & ew_QuotedValue(CustomerId.SessionValue, EW_DATATYPE_NUMBER)
			Else
				DetailFilter = ""
				Exit Property
			End If
	...
	' Master filter
	Public Property Get SqlMasterFilter_Customers
		SqlMasterFilter_Customers = "[CustomerID]=@CustomerID@"
	End Property

	' Detail filter
	Public Property Get SqlDetailFilter_Customers
		SqlDetailFilter_Customers = "[CustomerId]=@CustomerId@"
	End Property
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Sub LoadListRowValues(RsRow)
		...
		PromoCodeUsed.DbValue = RsRow("PromoCodeUsed")
	End Sub
	...
	Sub RenderListRow()
		...
		If Not IsNull(payment_status.CurrentValue) Then
			Select Case payment_status.CurrentValue
				...
				Case "Cancelled"
					payment_status.ViewValue = ew_IIf(payment_status.FldTagCaption(5) <> "", payment_status.FldTagCaption(5), "Cancelled")
				Case Else
		...
		' EmailSent
		If Not IsNull(EmailSent.CurrentValue) Then
			Select Case EmailSent.CurrentValue
				Case "confirm"
					EmailSent.ViewValue = ew_IIf(EmailSent.FldTagCaption(1) <> "", EmailSent.FldTagCaption(1), "Confirm")
				Case Else
					EmailSent.ViewValue = EmailSent.CurrentValue
			End Select
		Else
			EmailSent.ViewValue = Null
		End If
		...
		' PromoCodeUsed
		PromoCodeUsed.ViewValue = PromoCodeUsed.CurrentValue
		PromoCodeUsed.ViewCustomAttributes = ""
		...
		If Not ew_Empty(CustomerId.CurrentValue) Then
			CustomerId.HrefValue = "Customersedit.asp?CustomerID=" & CustomerId.CurrentValue
			CustomerId.LinkAttrs.AddAttribute "target", "", True ' Add target
			If Export <> "" Then CustomerId.HrefValue = ew_ConvertFullUrl(CustomerId.HrefValue)
		Else
		...
		' PromoCodeUsed
		PromoCodeUsed.LinkCustomAttributes = ""
		PromoCodeUsed.HrefValue = ""
		PromoCodeUsed.TooltipValue = ""
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					Call XmlDoc.AddField("PromoCodeUsed", PromoCodeUsed.ExportValue(Export, ExportOriginalValue))
				Else
					Call XmlDoc.AddField("OrderId", OrderId.ExportValue(Export, ExportOriginalValue))
					Call XmlDoc.AddField("CustomerId", CustomerId.ExportValue(Export, ExportOriginalValue))
					Call XmlDoc.AddField("InvoiceId", InvoiceId.ExportValue(Export, ExportOriginalValue))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				...
				Call Doc.ExportCaption(PromoCodeUsed)
			Else
				Call Doc.ExportCaption(OrderId)
				Call Doc.ExportCaption(CustomerId)
				Call Doc.ExportCaption(InvoiceId)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					Call Doc.ExportField(PromoCodeUsed)
				Else
					Call Doc.ExportField(OrderId)
					Call Doc.ExportField(CustomerId)
					Call Doc.ExportField(InvoiceId)
...
'			End If
	...
	' Field PromoCodeUsed
	Private m_PromoCodeUsed

	Public Property Get PromoCodeUsed()
		If Not IsObject(m_PromoCodeUsed) Then
			Set m_PromoCodeUsed = NewFldObj("Orders", "Orders", "x_PromoCodeUsed", "PromoCodeUsed", "[PromoCodeUsed]", 202, 8, "", False, False, "FORMATTED TEXT")
		End If
		Set PromoCodeUsed = m_PromoCodeUsed
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		...
		If IsObject(m_PromoCodeUsed) Then Set m_PromoCodeUsed = Nothing
