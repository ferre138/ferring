...
<p class="aspmaker ewTitle"><%= Language.Phrase("LoginPage") %></p>
<% login.ShowMessage %>
<form action="<%= ew_CurrentPage %>" method="post" onsubmit="return login.ValidateForm(this);">
<table border="0" cellspacing="0" cellpadding="4">
	<tr>
		<td><span class="aspmaker"><%= Language.Phrase("UserName") %></span></td>
		<td><span class="aspmaker"><input type="text" name="username" id="username" size="20" value="<%= login.sUsername %>"></span></td>
	</tr>
	<tr>
		<td><span class="aspmaker"><%= Language.Phrase("Password") %></span></td>
		<td><span class="aspmaker"><input type="password" name="password" id="password" size="20"></span></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
		<td><span class="aspmaker">
		<label><input type="radio" name="rememberme" id="rememberme" value="a"<% If login.sLoginType = "a" Then %> checked="checked"<% End If %>><%= Language.Phrase("AutoLogin") %></label><br>
		<label><input type="radio" name="rememberme" id="rememberme" value="u"<% If login.sLoginType = "u" Then %>  checked="checked"<% End If %>><%= Language.Phrase("SaveUserName") %></label><br>
		<label><input type="radio" name="rememberme" id="rememberme" value=""<% If login.sLoginType = "" Then %> checked="checked"<% End If %>><%= Language.Phrase("AlwaysAsk") %></label><br>
		</span></td>
	</tr>
	<tr>
		<td colspan="2" align="center"><span class="aspmaker"><input type="submit" name="submit" id="submit" value="<%= ew_BtnCaption(Language.Phrase("Login")) %>"></span></td>
	</tr>
</table>
</form>
<br>
