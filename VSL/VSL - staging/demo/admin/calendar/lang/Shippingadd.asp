...
' Define page object
Dim Shipping_add
Set Shipping_add = New cShipping_add
Set Page = Shipping_add

' Page init processing
Call Shipping_add.Page_Init()

' Page main processing
Call Shipping_add.Page_Main()
%>
<!--#include file="header.asp"-->
<script type="text/javascript">
<!--
// Create page object
var Shipping_add = new ew_Page("Shipping_add");
// page properties
Shipping_add.PageID = "add"; // page ID
Shipping_add.FormID = "fShippingadd"; // form ID
var EW_PAGE_ID = Shipping_add.PageID; // for backward compatibility
// extend page with ValidateForm function
Shipping_add.ValidateForm = function(fobj) {
...
Shipping_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!
 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
<% If EW_CLIENT_VALIDATE Then %>
Shipping_add.ValidateRequired = true; // uses JavaScript validation
<% Else %>
Shipping_add.ValidateRequired = false; // no JavaScript validation
<% End If %>
//-->
</script>
<script type="text/javascript">
...
<% Shipping_add.ShowPageHeader() %>
<p class="aspmaker ewTitle"><%= Language.Phrase("Add") %>&nbsp;<%= Language.Phrase("TblTypeTABLE") %><%= Shipping.TableCaption %></p>
<p class="aspmaker"><a href="<%= Shipping.ReturnUrl %>"><%= Language.Phrase("GoBack") %></a></p>
<% Shipping_add.ShowMessage %>
<form name="fShippingadd" id="fShippingadd" action="<%= ew_CurrentPage %>" method="post" onsubmit="return Shipping_add.ValidateForm(this);">
<p>
...
Class cShipping_add
	...
	Sub Page_Main()
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			...
			If Not ValidateForm() Then
				Shipping.CurrentAction = "I" ' Form error, reset action
		...
		Else
			...
			If Request.QueryString("AddressID").Count > 0 Then
				...
				Call Shipping.SetKey("AddressID", Shipping.AddressID.CurrentValue) ' Set up key
			Else
				Call Shipping.SetKey("AddressID", "") ' Clear key
				CopyRecord = False
			End If
			If CopyRecord Then
		...
		Select Case Shipping.CurrentAction
			...
			Case "A" ' Add new record
				...
				If AddRow(OldRecordset) Then ' Add successful
					...
					If ew_GetPageName(sReturnUrl) = "Shippingview.asp" Then sReturnUrl = Shipping.ViewUrl ' View paging, return to view page with keyurl directly
		...
		Shipping.RowType = EW_ROWTYPE_ADD ' Render add type
	...
	Function LoadDefaultValues()
		...
		Shipping.ship_FirstName.CurrentValue = Null
		Shipping.ship_FirstName.OldValue = Shipping.ship_FirstName.CurrentValue
		Shipping.ship_LastName.CurrentValue = Null
		Shipping.ship_LastName.OldValue = Shipping.ship_LastName.CurrentValue
		Shipping.ship_Address.CurrentValue = Null
		Shipping.ship_Address.OldValue = Shipping.ship_Address.CurrentValue
		Shipping.ship_City.CurrentValue = Null
		Shipping.ship_City.OldValue = Shipping.ship_City.CurrentValue
		Shipping.ship_Province.CurrentValue = Null
		Shipping.ship_Province.OldValue = Shipping.ship_Province.CurrentValue
		Shipping.ship_PostalCode.CurrentValue = Null
		Shipping.ship_PostalCode.OldValue = Shipping.ship_PostalCode.CurrentValue
		Shipping.ship_Country.CurrentValue = Null
		Shipping.ship_Country.OldValue = Shipping.ship_Country.CurrentValue
		Shipping.ship_EmailAddress.CurrentValue = Null
		Shipping.ship_EmailAddress.OldValue = Shipping.ship_EmailAddress.CurrentValue
		Shipping.HomePhone.CurrentValue = Null
		Shipping.HomePhone.OldValue = Shipping.HomePhone.CurrentValue
		Shipping.WorkPhone.CurrentValue = Null
		Shipping.WorkPhone.OldValue = Shipping.WorkPhone.CurrentValue
		Shipping.ship_Address2.CurrentValue = Null
		Shipping.ship_Address2.OldValue = Shipping.ship_Address2.CurrentValue
	End Function
	...
	Function AddRow(RsOld)
		...
		' Field CustomerId
		Call Shipping.CustomerId.SetDbValue(Rs, Shipping.CustomerId.CurrentValue, Null, (Shipping.CustomerId.CurrentValue&"" = ""))

		' Field ship_FirstName
		Call Shipping.ship_FirstName.SetDbValue(Rs, Shipping.ship_FirstName.CurrentValue, Null, False)

		' Field ship_LastName
		Call Shipping.ship_LastName.SetDbValue(Rs, Shipping.ship_LastName.CurrentValue, Null, False)

		' Field ship_Address
		Call Shipping.ship_Address.SetDbValue(Rs, Shipping.ship_Address.CurrentValue, Null, False)

		' Field ship_City
		Call Shipping.ship_City.SetDbValue(Rs, Shipping.ship_City.CurrentValue, Null, False)

		' Field ship_Province
		Call Shipping.ship_Province.SetDbValue(Rs, Shipping.ship_Province.CurrentValue, Null, False)

		' Field ship_PostalCode
		Call Shipping.ship_PostalCode.SetDbValue(Rs, Shipping.ship_PostalCode.CurrentValue, Null, False)

		' Field ship_Country
		Call Shipping.ship_Country.SetDbValue(Rs, Shipping.ship_Country.CurrentValue, Null, False)

		' Field ship_EmailAddress
		Call Shipping.ship_EmailAddress.SetDbValue(Rs, Shipping.ship_EmailAddress.CurrentValue, Null, False)

		' Field HomePhone
		Call Shipping.HomePhone.SetDbValue(Rs, Shipping.HomePhone.CurrentValue, Null, False)

		' Field WorkPhone
		Call Shipping.WorkPhone.SetDbValue(Rs, Shipping.WorkPhone.CurrentValue, Null, False)

		' Field ship_Address2
		Call Shipping.ship_Address2.SetDbValue(Rs, Shipping.ship_Address2.CurrentValue, Null, False)
		...
		bInsertRow = Shipping.Row_Inserting(RsOld, Rs)
		If bInsertRow Then
		...
		If AddRow Then
			...
			Call Shipping.Row_Inserted(RsOld, RsNew)
		End If
