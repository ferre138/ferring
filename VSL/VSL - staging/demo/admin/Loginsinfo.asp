...
' ASPMaker configuration for Table Logins
Dim Logins
...
Class cLogins
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "zUserId", zUserId)
		Call ew_SetArObj(Fields, "Loginname", Loginname)
		Call ew_SetArObj(Fields, "Loginpass", Loginpass)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "Logins_Highlight"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(zUserId.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@zUserId@", ew_AdjustSql(zUserId.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "Loginslist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "Loginslist.asp"
	End Function

	' View url
	Public Function ViewUrl()
		ViewUrl = KeyUrl("Loginsview.asp", UrlParm(""))
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("Loginsedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("Loginsadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("Loginsdelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(zUserId.CurrentValue) Then
			sUrl = sUrl & "zUserId=" & zUserId.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("zUserId") ' UserId
	...
	Public Sub LoadListRowValues(RsRow)
		zUserId.DbValue = RsRow("UserId")
		Loginname.DbValue = RsRow("Loginname")
		Loginpass.DbValue = RsRow("Loginpass")
	End Sub
	...
	Sub RenderListRow()
		...
		zUserId.ViewValue = zUserId.CurrentValue
		zUserId.ViewCustomAttributes = ""

		' Loginname
		Loginname.ViewValue = Loginname.CurrentValue
		Loginname.ViewCustomAttributes = ""

		' Loginpass
		Loginpass.ViewValue = Loginpass.CurrentValue
		Loginpass.ViewCustomAttributes = ""

		' UserId
		zUserId.LinkCustomAttributes = ""
		zUserId.HrefValue = ""
		zUserId.TooltipValue = ""

		' Loginname
		Loginname.LinkCustomAttributes = ""
		Loginname.HrefValue = ""
		Loginname.TooltipValue = ""

		' Loginpass
		Loginpass.LinkCustomAttributes = ""
		Loginpass.HrefValue = ""
		Loginpass.TooltipValue = ""
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call XmlDoc.AddField("zUserId", zUserId.ExportValue(Export, ExportOriginalValue))
					Call XmlDoc.AddField("Loginname", Loginname.ExportValue(Export, ExportOriginalValue))
					Call XmlDoc.AddField("Loginpass", Loginpass.ExportValue(Export, ExportOriginalValue))
				Else
					Call XmlDoc.AddField("zUserId", zUserId.ExportValue(Export, ExportOriginalValue))
					Call XmlDoc.AddField("Loginname", Loginname.ExportValue(Export, ExportOriginalValue))
					Call XmlDoc.AddField("Loginpass", Loginpass.ExportValue(Export, ExportOriginalValue))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				Call Doc.ExportCaption(zUserId)
				Call Doc.ExportCaption(Loginname)
				Call Doc.ExportCaption(Loginpass)
			Else
				Call Doc.ExportCaption(zUserId)
				Call Doc.ExportCaption(Loginname)
				Call Doc.ExportCaption(Loginpass)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call Doc.ExportField(zUserId)
					Call Doc.ExportField(Loginname)
					Call Doc.ExportField(Loginpass)
				Else
					Call Doc.ExportField(zUserId)
					Call Doc.ExportField(Loginname)
					Call Doc.ExportField(Loginpass)
...
'			End If
	...
	Public Property Get zUserId()
		If Not IsObject(m_zUserId) Then
			Set m_zUserId = NewFldObj("Logins", "Logins", "x_zUserId", "UserId", "[UserId]", 3, 8, "", False, False, "FORMATTED TEXT")
			m_zUserId.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set zUserId = m_zUserId
	End Property
	...
	Public Property Get Loginname()
		If Not IsObject(m_Loginname) Then
			Set m_Loginname = NewFldObj("Logins", "Logins", "x_Loginname", "Loginname", "[Loginname]", 202, 8, "", False, False, "FORMATTED TEXT")
		End If
		Set Loginname = m_Loginname
	End Property
	...
	Public Property Get Loginpass()
		If Not IsObject(m_Loginpass) Then
			Set m_Loginpass = NewFldObj("Logins", "Logins", "x_Loginpass", "Loginpass", "[Loginpass]", 202, 8, "", False, False, "FORMATTED TEXT")
		End If
		Set Loginpass = m_Loginpass
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		If IsObject(m_zUserId) Then Set m_zUserId = Nothing
		If IsObject(m_Loginname) Then Set m_Loginname = Nothing
		If IsObject(m_Loginpass) Then Set m_Loginpass = Nothing
