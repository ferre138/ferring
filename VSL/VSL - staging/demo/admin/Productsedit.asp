...
' Define page object
Dim Products_edit
Set Products_edit = New cProducts_edit
Set Page = Products_edit

' Page init processing
Call Products_edit.Page_Init()

' Page main processing
Call Products_edit.Page_Main()
%>
...
Products_edit.FormID = "fProductsedit"; // form ID
...
Products_edit.MultiPage = new ew_MultiPage();
Products_edit.MultiPage.AddElement("x_ItemId", 1);
Products_edit.MultiPage.AddElement("x_Description", 1);
Products_edit.MultiPage.AddElement("x_Price", 1);
Products_edit.MultiPage.AddElement("x_Active", 1);
Products_edit.MultiPage.AddElement("x_Image", 1);
Products_edit.MultiPage.AddElement("x_Sizes", 1);
Products_edit.MultiPage.AddElement("x_Image_Thumb", 1);
Products_edit.MultiPage.AddElement("x_ProductName", 1);
Products_edit.MultiPage.AddElement("x_ItemNo", 1);
Products_edit.MultiPage.AddElement("x_UPC", 1);
Products_edit.MultiPage.AddElement("x_Price_rebate", 1);
Products_edit.MultiPage.AddElement("x_fDescription", 2);
Products_edit.MultiPage.AddElement("x_fImage", 2);
Products_edit.MultiPage.AddElement("x_fSizes", 2);
Products_edit.MultiPage.AddElement("x_fImage_Thumb", 2);
Products_edit.MultiPage.AddElement("x_fProductName", 2);
//-->
</script>
<script type="text/javascript">
<!--
...
<% Products_edit.ShowPageHeader() %>
<p class="aspmaker ewTitle"><%= Language.Phrase("Edit") %>&nbsp;<%= Language.Phrase("TblTypeTABLE") %><%= Products.TableCaption %></p>
<p class="aspmaker"><a href="<%= Products.ReturnUrl %>"><%= Language.Phrase("GoBack") %></a></p>
<% Products_edit.ShowMessage %>
<form name="fProductsedit" id="fProductsedit" action="<%= ew_CurrentPage %>" method="post" enctype="multipart/form-data" onsubmit="return Products_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="Products">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" cellpadding="0"><tr><td>
<div id="Products_edit" class="yui-navset">
	<ul class="yui-nav">
...
<% If Products.ItemId.Visible Then ' ItemId %>
	<tr id="r_ItemId"<%= Products.RowAttributes %>>
		...
		<td<%= Products.ItemId.CellAttributes %>><span id="el_ItemId">
<div<%= Products.ItemId.ViewAttributes %>><%= Products.ItemId.EditValue %></div>
<input type="hidden" name="x_ItemId" id="x_ItemId" value="<%= Server.HTMLEncode(Products.ItemId.CurrentValue&"") %>">
</span><%= Products.ItemId.CustomMsg %></td>
	</tr>
<% End If %>
...
<% If Products.Price_rebate.Visible Then ' Price_rebate %>
	<tr id="r_Price_rebate"<%= Products.RowAttributes %>>
		...
		<td<%= Products.Price_rebate.CellAttributes %>><span id="el_Price_rebate">
<input type="text" name="x_Price_rebate" id="x_Price_rebate" size="30" maxlength="50" value="<%= Products.Price_rebate.EditValue %>"<%= Products.Price_rebate.EditAttributes %>>
</span><%= Products.Price_rebate.CustomMsg %></td>
	</tr>
<% End If %>
</table>
</div>
...
Class cProducts_edit
	...
	Sub Page_Main()
		...
		If ObjForm.GetValue("a_edit")&"" <> "" Then
			Products.CurrentAction = ObjForm.GetValue("a_edit") ' Get action code
		...
		' Check if valid key
		If Products.ItemId.CurrentValue = "" Then Call Page_Terminate("Productslist.asp") ' Invalid key, return to list
	...
	Function LoadFormValues()
		...
		If Not Products.ItemId.FldIsDetailKey Then Products.ItemId.FormValue = ObjForm.GetValue("x_ItemId")
		...
		If Not Products.Price_rebate.FldIsDetailKey Then Products.Price_rebate.FormValue = ObjForm.GetValue("x_Price_rebate")
	...
	Function RestoreFormValues()
		Call LoadRow()
		Products.ItemId.CurrentValue = Products.ItemId.FormValue
		...
		Products.Price_rebate.CurrentValue = Products.Price_rebate.FormValue
	...
	Sub RenderRow()
		...
		ElseIf Products.RowType = EW_ROWTYPE_EDIT Then ' Edit row
			...
			' ItemId
			Products.ItemId.EditCustomAttributes = ""
			Products.ItemId.EditValue = Products.ItemId.CurrentValue
			Products.ItemId.ViewCustomAttributes = ""
			...
			' Price_rebate
			Products.Price_rebate.EditCustomAttributes = ""
			Products.Price_rebate.EditValue = ew_HtmlEncode(Products.Price_rebate.CurrentValue)
	...
	Function EditRow()
		...
		Else
			...
			' Field Description
			Call Products.Description.SetDbValue(Rs, Products.Description.CurrentValue, Null, Products.Description.ReadOnly)

			' Field Price
			Call Products.Price.SetDbValue(Rs, Products.Price.CurrentValue, Null, Products.Price.ReadOnly)

			' Field Active
			boolwrk = Products.Active.CurrentValue
			If boolwrk&"" <> "1" And boolwrk&"" <> "0" Then boolwrk = ew_IIf(boolwrk&"" <> "", "1", "0")
			Call Products.Active.SetDbValue(Rs, boolwrk, Null, Products.Active.ReadOnly)
			...
			If IsNull(Products.Image.Upload.Value) Then
				Rs("Image") = Null
			Else
				If Products.Image.Upload.FileName = Products.Image.Upload.DbValue Then ' Upload file name same as old file name
					Rs("Image") = Products.Image.Upload.FileName
				Else
			...
			' Field Sizes
			Call Products.Sizes.SetDbValue(Rs, Products.Sizes.CurrentValue, Null, Products.Sizes.ReadOnly)
			...
			If IsNull(Products.Image_Thumb.Upload.Value) Then
				Rs("Image_Thumb") = Null
			Else
				If Products.Image_Thumb.Upload.FileName = Products.Image_Thumb.Upload.DbValue Then ' Upload file name same as old file name
					Rs("Image_Thumb") = Products.Image_Thumb.Upload.FileName
				Else
			...
			' Field ProductName
			Call Products.ProductName.SetDbValue(Rs, Products.ProductName.CurrentValue, Null, Products.ProductName.ReadOnly)

			' Field ItemNo
			Call Products.ItemNo.SetDbValue(Rs, Products.ItemNo.CurrentValue, Null, Products.ItemNo.ReadOnly)

			' Field UPC
			Call Products.UPC.SetDbValue(Rs, Products.UPC.CurrentValue, Null, Products.UPC.ReadOnly)

			' Field Price_rebate
			Call Products.Price_rebate.SetDbValue(Rs, Products.Price_rebate.CurrentValue, Null, Products.Price_rebate.ReadOnly)

			' Field fDescription
			Call Products.fDescription.SetDbValue(Rs, Products.fDescription.CurrentValue, Null, Products.fDescription.ReadOnly)
			...
			If IsNull(Products.fImage.Upload.Value) Then
				Rs("fImage") = Null
			Else
				If Products.fImage.Upload.FileName = Products.fImage.Upload.DbValue Then ' Upload file name same as old file name
					Rs("fImage") = Products.fImage.Upload.FileName
				Else
			...
			' Field fSizes
			Call Products.fSizes.SetDbValue(Rs, Products.fSizes.CurrentValue, Null, Products.fSizes.ReadOnly)
			...
			Else
				If Products.fImage_Thumb.Upload.FileName = Products.fImage_Thumb.Upload.DbValue Then ' Upload file name same as old file name
					Rs("fImage_Thumb") = Products.fImage_Thumb.Upload.FileName
				Else
			...
			' Field fProductName
			Call Products.fProductName.SetDbValue(Rs, Products.fProductName.CurrentValue, Null, Products.fProductName.ReadOnly)
			...
			bUpdateRow = Products.Row_Updating(RsOld, Rs)
			If bUpdateRow Then
		...
		If EditRow Then
			Call Products.Row_Updated(RsOld, RsNew)
		End If
		Rs.Close
		Set Rs = Nothing
