...
if(cint(session("invalidtry"))>10) then 
	...
	if(code<>"") then
		...
		strSql = strSql & " WHERE (((Discountcodes.DiscountCode)='"& mid(code,1,5) &"') AND ((Discountcodes.Active)=True) AND ((Discountcodes.used)=False)) AND ((Discountcodes.used)=False) AND ((DiscountTypes.StartDate)<Now()) AND ((DiscountTypes.EndDate)>Now());"
