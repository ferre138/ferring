...
Session("Anniversary_sale")=False
if((Month(now())<=12)  and (year(now())=2017)) then
   if( (day(now())<=18)) then 
        Session("Anniversary_sale")=true
    end if
end if
...
function getAddress(c)
    ...
    <tr> 
        ...
        <option value="" selected > Please Select </option> 
                  ...
                  <tr> 
                    <td colspan="2" class="vslcss"><div class="t">
                       ...
                       %>
						...
						<p style="background-color: #FFFFCC;"><font color="#FF0000" size="+1" >Until December 18th, 2017 or while supplies last</font></p>
						...
						<p style="background-color: #FFFFCC;"><font color="#FF0000"  >Until December 8th, 2017, with every 3 cartons of VSL#3  you purchase, you will receive a 4th carton at no charge while supplies last.  
                 ...
                 <span class="ewmsg">LAST WEEK TO ORDER 2017<br/>We will be closed for the Holiday Season from December 18th ,2017 to January 2nd, 2018.
...
<% if(Session("Anniversary_sale")) then
    if(int(QT/3)>0) then
%>

    <tr >
          <td width="50%"><b>Anniversary sale Free Box</b></td>
          <td width="16%"><div align="right"><b>0</b></div></td>
          <td width="12%" align="center"><div align="center"><%=int(QT/3)%></div></td>
          <td width="10%"><div align="right">0</div></td>
     </tr>
...
if(cint(session("invalidtry"))>10) then 
	...
	if(code<>"") then
		...
		dim rs,NewCustomer
		Set rs = Server.CreateObject("ADODB.Recordset")
		strSql ="SELECT Discountcodes.DiscountCode, DiscountTypes.DiscountTitle, DiscountTypes.DiscountType, DiscountTypes.freeShipping, DiscountTypes.FreePerQty, DiscountTypes.DiscountPerc,DiscountTypes.SpecialPrice, Orders.PromoCodeUsed "
		...
		if(not rs.eof) then 
			...
			'if(true)  then
                ...
                Session("discount_am")=rs.fields("DiscountPerc")
                Session("DiscountTitle")=rs.fields("DiscountTitle")
				checkCustomer=true
