	...
	if(not rs.eof) then 
		...
		if(applyDiscount) then
			if(session("specialxprice")<>""	) then 
				...
				t= ": Special price using promo : " & session("promocode") & " : " & p
			end if
		end if
		'p=0.10		
 %>
		<input type="hidden" name="item_name_<%=(i+1)%>" value="<%= rs("Description")%><%=t%>">
...
%>
			<input type="hidden" name="item_name_<%=UBound(arr)+k%>" value="<%= " Free.." & rs("Description")%>">
...
<input type="hidden" name="cancel_return" value="https://www.vsl3.ca/Cancel_order.asp?token=<%=OrderNum%>&txt=<%=CustId%>">
